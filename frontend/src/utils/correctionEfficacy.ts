...
const STORE_KEY = "asanaai.correctionEfficacy.v1";
...
export class CorrectionEfficacyTracker {
  ...
  private save() {
    try {
      ...
      this.failures.forEach((v, k) => failures.push([k, v]));
      window.localStorage.setItem(STORE_KEY, JSON.stringify({
        failures,
        history: this.history.slice(-50),
      }));
    } catch {
      /* storage unavailable; the loop still works in-memory */
    }
  }
  ...
  private load() {
    try {
      ...
      if (Array.isArray(p?.failures)) this.failures = new Map(p.failures);
