"""AsanaAI MLP v4 -- all three heads, on the full 10x photo corpus.

v4 adds the public-dataset corpus (5,655 photos extracted from four public
yoga datasets) to v3's Commons+Openverse set, taking the photo pool from 587
to roughly 6,200 and, far more importantly, giving the starved classes real
support for the first time: chaturanga 0 -> 271, seated_forward 5 -> 268,
halfway_lift 0 -> 49, upward_dog 1 -> 51, plank 14 -> 891.

Those five classes previously scored 0% for the obvious reason that they had
almost no training examples. This is the run that tests whether that was the
whole story.

Everything else is unchanged from v3, including the grouped-by-video split
and the 3-head loss.
...
def main():
    ...
    pub_npz = find("public_corpus.npz") or find("public_corpus_partial.npz")
    pub_man = find("public_corpus_manifest.json")
    ...
    # The public-dataset corpus is where the starved classes finally come
    # from: chaturanga 0 -> 271, seated_forward 5 -> 268, halfway_lift 0 -> 49,
    # plank 14 -> 891. Its images are keyed by a hash of DECODED PIXELS, which
    # is what lets it union safely with the Commons/Openverse corpora -- those
    # datasets overlap byte-for-byte under different filenames, and 848
    # duplicates were caught on extraction alone.
    if pub_npz:
        a3, l3, s3, i3 = load_corpus(pub_npz, pub_man)
        print(f"public: {len(l3)} photos", flush=True)
    else:
        print("WARNING: public corpus absent -- this run cannot test whether "
              "the starved classes improved", flush=True)
        a3 = np.zeros((0,15), np.float32); l3 = np.array([]); s3 = np.array([]); i3 = []

    A, L, S, I, seen = [], [], [], [], set()
    for ang, lab, sp, ids in ((a1,l1,s1,i1), (a2,l2,s2,i2), (a3,l3,s3,i3)):
