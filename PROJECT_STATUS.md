...
> * **True Spatio-Temporal Graph Convolutional Network (ST-GCN) Architecture**: Upgraded [train_stgcn_gpu.py](file:///home/anamitra/Projects_and_Code/Scripts_and_Source/train_stgcn_gpu.py) and backend models to a genuine ST-GCN. It defines a 33-joint MediaPipe skeletal graph topology using a normalized adjacency matrix $A_{norm} = D^{-1/2}(A + I)D^{-1/2}$, performing spatial convolutions (`SpatialGraphConv` via `torch.einsum`) combined with temporal convolutions (`TemporalConv` over 60 frames) and residual skip pathways (`STGCNBlock`). It includes pelvis-centered scale-normalization for coordinates, a **Cosine Annealing** LR scheduler, and saves checkpoints based on best **Validation Accuracy**. Uses a **batch size of 64** and **label smoothing of 0.1** to handle majority-voted sequence label noise. Training duration is set to **120 epochs** to allow full convergence.
